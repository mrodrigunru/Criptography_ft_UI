...
/**
 * Clase que define la interfaz de firma del programa
 * 
 * @author Manuel Rodriguez Rodriguez
 * @version 02/05/2023
 */
public class UIFirma extends JFrame implements ActionListener{
...
//		name = nombre;
		...
		label2 = new JLabel("Inserta el nombre con extensión del fichero a firmar: ");
		...
		label = new JLabel("Elige el algoritmo de firma");
		label.setBounds(20, 90, 250, 30);
		add(label);
		type = new JComboBox<String>();
		type.setBounds(20, 120, 200, 30);
		add(type);
		type.addItem("SHA1withRSA");
		type.addItem("MD2withRSA");
		type.addItem("MD5withRSA");
		type.addItem("SHA1withDSA");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == botonAceptar) {
			...
			String alg = (String) type.getSelectedItem().toString();
			name = field.getText();
			...
			if(!firma.firmar(alg,ficheroCif)) {
				throw new RuntimeException("No se ha podido firmar el archivo.");
			}
			
			JLabel bien = new JLabel("El fichero se ha firmado correctamente");
